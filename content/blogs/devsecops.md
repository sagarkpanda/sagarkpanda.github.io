...
description: "Hands-on DevSecOps on EKS covering application security, Kubernetes policy, runtime detection, and dynamic application testing."
date: "2026-08-13"
...
> ZAP which was part of OWASP, is not part of checkmarx, still I call it OWASP ZAP
...
Processes interact with Linux through **system calls**. File access, process creation, network activity and many other operations eventually involve syscalls.

That makes them useful signals for runtime security.

[Screenshot 1: Local Ubuntu - /etc/shadow access attempt in terminal]

### eBPF

We also looked at **eBPF** as part of understanding how Falco can observe activity close to the Linux kernel.

The important distinction is that Falco is not looking at the application source or Kubernetes YAML. It is observing what is happening while the workload is actually running.
...
You can also create custom rules and have them run for exmaple using whoami or ls etc for understanding it beofre implementing in k8s.
...
Ther's also a test workload under archive called falco-trigger.yml, this can be used as the image is in my repo and that's part of trusted registry in the policy that I have.

First apply the workload by commneting anything that voliates polciy like requests and limits or namespace. You would see that it would fail with violations shows, if you uncommnet them and try again, it is complinat and would work just fine.
...
{{< figure src="https://i.ibb.co/Jw6B4cSD/engmnts-auto-created-by-api.png" alt="engagements" width="1000" height="600" title="auto created engagemnts" >}}
      ...
      - name: Upload ZAP Results to DefectDojo
        ...
        run: |
          curl -X POST \
            ...
            -F "product_type_name=SP Org" \
            -F "product_name=Otel Labs" \
            -F "engagement_name=App Security Engagement" \
            -F "test_title=OWASP ZAP DAST"
```
...
The gh action zap can also create a issue in gh if you set that to true. You also get a html report for easy reading.
